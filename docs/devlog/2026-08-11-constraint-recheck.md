...
- [~] **C-6**: 適合そのものは未着手だが、**2026-08-12 に「何を要求されるのか」を probe で全部洗い出した**（要求プロパティ名・型・optional 可否、入れ子の `locationTrigger` / `locationTriggerEvent` まで。probe はビルドが通る形まで到達して削除済み）。据え置き理由だった「マクロ生成 init と自前 init が衝突する」は**誤り**と判明（マクロは conformance 2 つを生やすだけで init を生成しない）。残る本当の障害は ①`list` が非 optional ②`dueDate` が `DateComponents` ③`locationTrigger` が `PlaceDescriptor` を `@Property` に強制 → [[placedescriptor-ssu-workaround]] の SSU バグに正面衝突、の 3 点。詳細と要求プロパティ表: `docs/devlog/03-app-intents-core.md`。
- [ ] **C-8**: Control のフィードバック設計は 2026-08-12 に見直し済み（Toggle 化 + 成功通知廃止 + `.controlWidgetStatus` 撤去、経緯は `docs/devlog/06-control-widget-ios26.md`）。**2026-08-12 に iOS 27 シミュレータの Control Center で実測**: Toggle の on/off 表示は意図どおり（`value` が `To Do` ↔ `Completed, Selected` で切り替わる）。同時に**バグを 1 件発見して修正** — トグルで完了にしても隣の `TodoCountControl`（未完了数）が古い値のまま止まっていた。`WidgetReloader` が `WidgetCenter.shared.reloadAllTimelines()` しか呼んでおらず、`ControlCenter.shared.reloadAllControls()` が抜けていたため（ホームウィジェットとコントロールは別 API）。修正後は連動して更新される。残るのは `.promptsForUserConfiguration()` の設定フローと失敗時のエラー通知（今回はコントロールが設定済みで通らなかった）。詳細: `docs/devlog/06-control-widget-ios26.md`
