...
- 本文 L22–209 は **7,777 字 / 84 面**（`///` 区切りで機械カウント）
...
| 4 | SiriKit 2016 → App Intents 2022 | #15–24 | L56–78 | 775 | 1.6 | 2.0 |
| 5 | 2023（ウィジェット） | #25–28 | L80–86 | 310 | 0.6 | 0.8 |
| 6 | 2024（Apple Intelligence / Domains） | #29–31 | L88–92 | 251 | 0.5 | 0.8 |
| 7 | 2025（Snippets / Visual Intelligence） | #32–35 | L94–100 | 235 | 0.5 | 0.7 |
| 8 | 2026 + 年表の総括 | #36–40 | L102–110 | 415 | 0.8 | 1.3 |
| 9 | Liquid Glass の仮説 → App Intents | #41–51 | L111–135 | 1,002 | 2.0 | 2.5 |
...
| 13 | 動詞・名詞 → モデルベース UI デザイン | #64–70 | L166–179 | 579 | 1.2 | 1.4 |
| 14 | 実践（`Button(intent:)` / Service / DI） | #71–80 | L181–200 | 879 | 1.8 | 2.5 |
| 15 | サイレント失敗 → AppIntentsTesting | #81–83 | L202–207 | 357 | 0.7 | 0.9 |
| | **合計（締め除く）** | | | **7,777** | **15.7** | **≈ 20.5** |
...
| **App Schema を 30 秒 → 1.5 分に**（2 層 + reminders 適合の実話 + FB24570185） | +1 分 | B-12 / G-6 |
| **砂時計の図を 1 枚**（Clean Architecture との対比） | +1 分 | C-8 / 骨子② T07b |
| **「1 つの Intent が 6 面に出る」実演**（アンケートで「知らない」が多かった場合の前半の厚み） | +3 分 | 骨子② T05 |
| **質疑が別枠なので、まさかりを本編で先に潰す**（OOUI / スキーマ必須論 / アクセシビリティ代替論を各 1 行） | +1 分 | F |
...
| パターン | 構成 | 尺 | 何が強いか |
|---|---|---:|---|
| ⭐ **(e) 第 2 部として立てる**（推奨） | **D-1（6）+ D-2（3.5）+ D-4（2.5）+ D-3 の要点だけ（2）+ D-5（1.5）+ D-6（0.5）** | **16 分** | 実測ベースの尺にちょうど収まる。**下げ（つまずき）→ さらに下げ（代償）→ 上げ（AI 時代）→ 持ち帰り（デザイン + 行動）** で終わりが上向く。CfP のアジェンダ 3・4・5 を全部回収 |
...
3. **D-4 AI 時代**（2.5 分）— ここで上げに転じる。CfP の最後の 1 行を回収するのはここ
4. **D-3 の要点だけ**（2 分）— 「最も制約の厳しい面から設計する / メインアプリの画面は最後」。
   **持ち帰りとして一番効くのはここ**（画面から設計する習慣の逆なので）
5. **D-5 + D-6**（2 分）— 最初の一歩 4 段 → 冒頭の仮説の回収で閉じる

- ⚠️ **D-1 を 6 分に伸ばすなら、C-5（テスト節の組み替え）は必須**。現行の #82 が
  「AppIntentsTesting で解決しました」で閉じてしまうと、直後に「動かなかった話」を 6 分やるのが矛盾する
- 余りが出たら A-5 の追加候補（App Schema を 1.5 分 / 砂時計の図 / 6 面の実演）から足す。
  **逆に本番で押していたら D-3 を落とす**（#64–70 で写像の話は済んでいるので、ここが一番落としやすい）
...
> （入れ先: OOUI → #69 付近 / スキーマ必須論 → #38（B-12）/ アクセシビリティ代替論 → #49（B-5））。
