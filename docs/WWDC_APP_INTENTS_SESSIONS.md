...
| `RelevantEntities.updateEntities(_:for:)` 強化 | 文脈に応じた Entity 寄付。RC 27A266a の公開 SDK で確認できる context は `.audio(.nowPlaying)`（[現行の制約](insights/03-app-intents-core.md#relevantentities-は-todo-ドメインに不適合)） |
