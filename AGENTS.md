...
12. **確認はビルドの成否ではなくメタデータで行う**。「どの面が何を提示するか」は
    **呼出元だけ変えて同じ Intent を走らせて**確定させる（肯定リストから推論しない）。
    ただし**そのメタデータもクリーンビルドで見る** — インクリメンタルビルドではパッケージ側の
    `*.appintents` が再生成されず、統合メタデータに古い値が混ざる（出力を手で消しても作り直されない）
13. **`AppIntentsPackage` を宣言しない**（パッケージが静的リンクである限り）。宣言が足す
    `extract.packagedata` はマングル名での実行時参照で、**TestFlight / App Store 配布だと
    App Intents がバンドルごと読み込まれなくなる**（Xcode 経由のインストールでは再現しない）
14. **生成物が正しいことは、配布したものが読まれることを意味しない**。ローカルビルドの
    メタデータで出した「害が無い」は**未検証**として扱い、面に関わる変更は TestFlight まで通す
...
- **GitHub issues** — これからやること（#30 実機検証 / #57 GM SDK 棚卸し / #68 未採用 API /
  #77 ja 翻訳の state レビュー / #132 提出前の残り確認）
