...
3. ⭐ **新しい intent donation の仕組み**（今年の新機能）:
   ...
   - ⚠️ **本プロジェクトは明示的な `donate()` を書いていない**（`perform()` が呼出元を判別できないため。骨子② T21b）。
     ただし **2026-08-30 の実測で、アプリ内 `Button(intent:)` はシステムが donation として記録していると分かった**
     → **この回答は T21b の「代償」ではなく「効能」側の材料**（§4-3）
...
### 4-3. 骨子② T21b（donation）— 「代償が重くなった」は 2026-08-30 の実測で裏返った
...
これを読んだ時点では「**中心設計を徹底すると、Apple が今年一番推している導線を 1 本落とす**」と
解釈していた。**この解釈は誤りだった**。実測（`donate()` を 1 行も書いていないのに、アプリ内
`Button(intent:)` のタップが donation に +1 で載る）により、落としていないことが分かっている。
経緯: [../devlog/2026-08-30-donation-observability.md](../devlog/2026-08-30-donation-observability.md)。

→ この `21:47` は **T21b のオチを強くする材料として今も使える**が、向きが逆になる:

> 「Apple は今年、donation を『Siri に影響を与える主要な手段』として推しています。中心設計は
> それを 1 本諦めることになる — と思っていたのですが、**測ったら諦めていませんでした**。
> `Button(intent:)` にした時点でシステムが記録してくれている。**公式サンプルが手で書いている
> donate は、UI が全部 intent 経由なら書くところが残らない**んです」
...
| **T21b（donation）** | §4-3 | **オチが裏返った**。今年一番の推し機能を諦めたと思ったら、`Button(intent:)` にした時点でシステムが記録していた |
