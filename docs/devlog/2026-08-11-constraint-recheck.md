...
**残るのは A-1 の Siri フレーズ routing 確認のみ**（Shortcuts アプリ側は実機で確認済み。Siri は機会があるときに 1 フレーズ言うだけ）。C-6 は SDK 側の SSU バグ待ちで、こちらから進められることは無い。

## 残タスク（実機検証 or 将来の機能作業が必要、このセッションでは未着手）

- [x] **A-1**: **採用して決着（2026-08-12）**。アプリ / Widget / LiveActivity / Watch App の 4 ターゲットに `includedPackages` 付き `AppIntentsPackage` を宣言する Apple 公式手順へ切り替えた。根拠: (1) 全バンドルの `Metadata.appintents` の件数が宣言の有無で**完全一致**（重複なし）、(2) 宣言した状態で **AppIntentsTesting 全テストがグリーン**（Siri / Shortcuts / Spotlight と同じインフラを通る）。**Shortcuts アプリ側は実機で確認済み**（アクション一覧・パラメータ表示が壊れていないこと。梯子の 2 段目クリア）。**残る未確認は App Shortcut の「フレーズ」ルーティング（Siri）のみ** — AppIntentsTesting は型名で intent を引くためフレーズ経路を通らない。実機 Siri でフレーズを 1 つ試せば確認でき、壊れていれば `*AppIntentsPackage.swift` の 4 ファイルを消せば戻る。詳細: `docs/devlog/03-app-intents-core.md`。
