...
リストは 2 つの画面に出るが、**別のことをする画面**なので分けてある。

| | 画面 | 置き場 | できること |
|---|---|---|---|
| 閲覧 | `TodoListsBrowseView` | 一覧のツールバー（`folder`） | どのリストを見るか選ぶ。選ぶと一覧に戻り、**ナビゲーションタイトルがそのリスト名になる** |
| 管理 | `ListManagementView` / `TagManagementView` | 設定の「管理」 | 作成 / 名称変更 / 色 / 統合 / 削除 |

同じ画面に両方を載せると、**ふだんの閲覧の動線上に破壊的な操作が並ぶ**。リストを選ぶのは
「やることを読む」作業の一部で、リストの名前を変えるのは稀な設定作業なので、頻度と可逆性が
違うものを 1 か所に混ぜない。設定側のフッターは「リストごとに見るには一覧のフォルダボタン」と
**やらないことを明示**している（両方が "Lists" という同じ語を使うため）。

リストの絞り込みは**フィルタメニューから外した**。フィルタ（状態）とタグは「同じものをどれだけ
絞るか」だが、リストは「何を見ているか」を変える軸で、タイトルが変わるものをメニューの 1 行に
畳むと変化の理由が見えない。

## 設定画面が持つもの（管理 / 連携 / 情報 / 診断）

`SettingsView` は 4 セクション。順番は「使う頻度が高いものから」。

| セクション | 中身 | なぜ設定にあるか |
|---|---|---|
| Manage | リスト / タグの管理画面への `NavigationLink` | 上記のとおり閲覧と分ける |
...
### 検索対象を広げたら「なぜヒットしたか」を行に出す

4 か所（タイトル / 説明 / リスト名 / タグ）を見るようにすると、**タグで当たった行が何も
当たっていない行と見分けが付かない**。行が自分の理由を説明する必要がある。

- **タイトルは一致部分を強調する**（`AttributedString.highlighting(_:in:)` で
  `inlinePresentationIntent = .stronglyEmphasized`）。理由がその場に見えるので説明文は要らない
- **タイトル以外で当たったときだけ** `TodoSearchReason` をキャプションに出す。タグはチップ、
  リストはフォルダ記号付き、説明は「説明に一致」。チップは 2 つまで + `+N`
- 判定は `TodoSearchMatch.make(for:term:organize:)` に閉じてあり、**絞り込みと同じ
  `localizedStandardContains(_:)`** を使う。行が「フィルタが使っていない理由」を主張しないため

---

## パッケージの UI コピーに `^[…](inflect: true)` を書かない

**パッケージのバンドルは、カタログが列挙しているロケールしか同梱しない。** カタログの
`sourceLanguage` が `en` でも、`en` の localization を持つキーが 1 つも無ければ
`UI_UI.bundle` に `en.lproj` は作られない。つまりソース言語では `.copy("…")` が
**キーそのもの**に落ちる。

素のコピーならキー = 英語なので無害だが、自動文法一致のマークアップだと**そのまま画面に出る**。

```
リストの一覧に「^[0 todo](inflect: true)」と表示される
```

アプリターゲット側は `en.lproj/Localizable.strings` を持つので同じキーでも動く（マークアップの
展開は実行時）。**パッケージだけが踏む**。ウィジェットの「残り n 件」は 2026-09-17 に気づくまで
英語環境でこの状態で出荷されていた。

**複数形はカタログに持たせる。** キーは素の `%lld todos` にして、`en` に
`plural.one` / `plural.other`（必要なら `plural.zero`）を入れる。ビルド後に
`UI_UI.bundle/en.lproj/Localizable.stringsdict` ができることで確認できる。
`%lld remaining` のように単複で形が変わらないものは `en` を足す必要すら無い（キーが正しい英語）。

守り方: `PackageCopyKeyTests`（4 パッケージのカタログにマークアップ入りキーが無いことを見る）。
...
#### バリエーションは語彙を変える。助詞や語尾だけ違うものを並べても増えない

en の `Snooze` / `Delay` / `Star` / `Favorite` のように、**元は別語彙で経路を増やしている**。
これを機械的に訳すと日本語では同じ語彙に寄ってしまい、バリエーションとして機能しない。

```
❌ 「〜をスヌーズ」「〜をスヌーズする」        する の有無しか違わない
❌ 「〜を削除」「〜から削除」                 助詞しか違わない
❌ 「〜をお気に入りに追加」「〜をお気に入りにする」
...
判定するときは**パラメータ構成が同じものだけを比べる**。`${filter}` 入りと無しで
