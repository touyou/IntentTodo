...
このファイルは、ドキュメント整備・公式ドキュメント検証を踏まえて起票すべき GitHub issue の一覧です。
`gh` CLI インストール後、下部のスクリプトで一括起票できます。

---

## カテゴリ A: プラットフォーム別検証

README の星取表で「🔲 未検証」となっている項目。

### A-1: macOS 実機検証

**Title**: `verify: macOS での Button(intent:) 全アクション動作検証`
**Labels**: `verification`, `platform:macOS`
...
#### 関連ファイル
- `Packages/TodoAppIntents/Sources/TodoAppIntents/Entities/TodoAppEntity.swift` (`IndexedEntity` iOS/macOS のみ)
...
**Labels**: `verification`, `platform:watchOS`
...
- [ ] `Button(intent:)` の watchOS 動作（※ watchOS では `Button(intent:role:)` が使えず Task パターンが必要 — insights/07 参照）
- [ ] コンプリケーション表示（Circular / Corner / Rectangular / Inline）
- [ ] SwiftData の watchOS 上での動作

#### watchOS 固有の注意点
insights/07-platform-specific.md に記載の通り、watchOS では `Button(intent:role:)` シグネチャが使えないため async パターンで代替している。この動作が正しいか検証すること。
...
**Labels**: `verification`, `platform:visionOS`
...
## カテゴリ B: Extension 別検証

### B-1: Control Center — QuickAdd 検証

**Title**: `verify: Control Center QuickAdd（通知パターン）の動作検証`
**Labels**: `verification`, `extension:control-center`
...
### B-2: Control Center — TodoCount 検証
...
### B-3: Live Activity 検証

**Title**: `verify: Live Activity（Dynamic Island + ロック画面）の動作検証`
**Labels**: `verification`, `extension:live-activity`
...
- [ ] `LiveActivityIntent` によるバックグラウンド Activity 開始（公式Doc: "you can only start a Live Activity while the app is in the foreground, unless you adopt App Intents and start the Live Activity using a LiveActivityIntent"）

#### 関連ファイル
- `IntentTodoLiveActivity/`
- `docs/insights/07-platform-specific.md`（LiveActivityIntent 公式Doc裏付け）

---

### B-4: Siri / Shortcuts 検証

**Title**: `verify: Siri フレーズと Shortcuts アプリからの Intent 実行検証`
**Labels**: `verification`, `extension:siri-shortcuts`

**Body**:

4 つの App Shortcuts がSiri と Shortcuts アプリから正しく動作するか検証。
...
- `docs/insights/03-app-intents-core.md`（フレーズのパラメータ型制限）
...
**Labels**: `verification`, `extension:spotlight`
...
#### 関連ファイル
- `Packages/TodoAppIntents/Sources/TodoAppIntents/Entities/TodoAppEntity.swift`（`#if os(iOS) || os(macOS)` で `IndexedEntity` 準拠）
...
**Labels**: `verification`, `extension:widget`
...
- `IntentTodoWidget/Configuration/TodoWidgetConfiguration.swift`

---

### B-7: watchOS コンプリケーション検証

**Title**: `verify: watchOS コンプリケーション全ファミリーの動作検証`
**Labels**: `verification`, `extension:complication`
...
## カテゴリ C: 新 API 活用によるコード改善

公式ドキュメント検証で発見した新 API を活用してコードベースをアップデートする。

### C-1: onAppIntentExecution によるIntent→UI連携の改善

**Title**: `enhance: onAppIntentExecution(_:perform:) で IntentAppState を補完/置換`
**Labels**: `enhancement`, `new-api`, `app-intents`
...
#### 背景
現在、Intent からアプリ UI を更新する場合、`IntentAppState`（App Group UserDefaults + Notification）を経由している。iOS 26 の `onAppIntentExecution` を使えば、View modifier で宣言的にハンドリングでき、コードがシンプルになる。
...
3. `IntentAppState` は Extension 間通信（Control Widget → App）に限定し、アプリ内の Intent → UI 連携は `onAppIntentExecution` へ段階的に移行

#### コード例
```swift
// Before: IntentAppState経由
...
// After: onAppIntentExecution で宣言的ハンドリング
struct LaunchAppIntent: OpenIntent, TargetContentProvidingIntent {
    @Parameter(title: "Target")
    var target: AppScreenTarget
...
NavigationStack { ... }
    .onAppIntentExecution(LaunchAppIntent.self) { intent in
        ...
        case .addTodo: showAddTodo = true
        case .todoList: /* navigate to list */
        }
    }
```

#### 関連ドキュメント
- `CLAUDE.md` の「onAppIntentExecution」セクション
- `docs/insights/04-ui-integration.md` の「onAppIntentExecution」セクション
- `docs/PLAN.md` の「onAppIntentExecution による宣言的 Intent → UI 連携」
...
### C-2: AppIntentSceneDelegate でシーンレベルの Intent ハンドリング

**Title**: `enhance: AppIntentSceneDelegate でシーン固有の Intent ハンドリングを追加`
**Labels**: `enhancement`, `new-api`, `app-intents`
...
#### 背景
`onAppIntentExecution` は View レベルのハンドリングだが、`AppIntentSceneDelegate` はシーンレベルで `scene(_:willPerformAppIntent:)` を受け取れる。これにより、アプリが複数シーンを持つ場合（macOS / visionOS / iPad マルチウィンドウ）に適切なシーンで Intent を処理できる。

#### 実装方針
1. `AppIntentSceneDelegate` を実装
2. `UISceneAppIntent` 準拠の Intent を定義（シーンターゲティング）
3. macOS / visionOS でのマルチウィンドウ対応を見据えた設計

#### 関連ドキュメント
- `CLAUDE.md`「onAppIntentExecution」セクションの AppIntentSceneDelegate 言及

---

### C-3: ControlWidgetButton OpenIntent のiOS 26バグ追跡

**Title**: `bug: ControlWidgetButton の OpenIntent initializer が iOS 26 で動作しない`
**Labels**: `bug`, `platform:iOS`, `extension:control-center`, `apple-bug`
...
- `perform()` は `.background` モードで確実に呼ばれることを確認済み
...
#### 公式ドキュメントの証拠
`ControlWidgetButton` のドキュメントに以下の initializer が記載:
> "Creates a button template for a control that launches an app"

#### 関連ファイル
- `IntentTodoWidget/Controls/` — 全 Control Widget
- `docs/insights/06-control-widget-ios26.md` — 詳細な検証記録

---

### C-4: Spotlight 検索属性の強化（CSSearchableItemAttributeSet）

**Title**: `enhance: TodoAppEntity の Spotlight 検索属性を CSSearchableItemAttributeSet で強化`
**Labels**: `enhancement`, `extension:spotlight`

**Body**:

現在の `IndexedEntity` 実装はデフォルトの `DisplayRepresentation` ベースのインデックスのみ。`CSSearchableItemAttributeSet` を使って検索属性を強化する。

#### 現状
```swift
extension TodoAppEntity: IndexedEntity {}  // デフォルト実装のみ
```
...
- `docs/PLAN.md`（Entity強化セクション）

---

### C-5: Intent Modes の高度な活用（.foreground(.deferred)）
...
`AddTodoIntent` を Siri / Shortcuts から実行する際、タイトルのみの簡易追加は `.background` で処理し、詳細設定が必要な場合のみ `continueInForeground()` でアプリを開くパターンを実装する。

#### 背景
iOS 26 の `supportedModes` で `[.background, .foreground(.deferred)]` を指定することで、実行時に動的にフォアグラウンド遷移を判断できる。
...
struct AddTodoIntent: AppIntent {
    ...
    @Parameter(title: "Needs Detail Setup")
    var needsDetailSetup: Bool?

    func perform() async throws -> some IntentResult {
        if needsDetailSetup == true {
...
### C-6: @Model Sendable 競合の明示的ドキュメント化とテスト

**Title**: `chore: @Model マクロの Sendable 自動準拠に関するテスト追加`
**Labels**: `documentation`, `testing`

**Body**:

`@Model` マクロが自動的に `Sendable` 準拠（`@available(*, unavailable)` 付き）を生成することを確認するテストを追加する。

#### 背景
公式ドキュメントの `@Model` マクロ定義:
```swift
@attached(extension, conformances: Observable, PersistentModel, Sendable)
```

insights/02 に記載済みだが、将来のSwiftData更新で挙動が変わる可能性があるため、コンパイル時チェックとして明示しておく。

#### 関連ファイル
- `Packages/Domain/Sources/Domain/Models/TodoItem.swift`
- `docs/insights/02-swiftdata-concurrency.md`

---

### C-7: NavigationIntents の TargetContentProvidingIntent 対応

**Title**: `enhance: OpenAddTodoIntent / OpenTodoListIntent に TargetContentProvidingIntent を追加`
**Labels**: `enhancement`, `new-api`, `app-intents`

**Body**:

現在の `OpenAddTodoIntent` と `OpenTodoListIntent` は単純な `.foreground` Intent だが、`TargetContentProvidingIntent` を追加して `onAppIntentExecution` との連携を可能にする。
...
これにより C-1 の `onAppIntentExecution` パターンとシームレスに連携できる。
...
`gh` CLI インストール後、以下を実行して全 issue を起票:
...
gh issue create --repo "$REPO" \
  ...
  --label "enhancement" \
  --body-file /dev/stdin <<'ISSUE_EOF'
iOS 26 の `AppIntentSceneDelegate` を活用し、シーンレベルで Intent をハンドリングする仕組みを追加する。macOS / visionOS / iPad マルチウィンドウでの適切なシーンでの Intent 処理を実現。
...
gh issue create --repo "$REPO" \
  ...
  --label "enhancement" \
  --body-file /dev/stdin <<'ISSUE_EOF'
Siri / Shortcuts から実行する際、タイトルのみなら `.background` で処理し、詳細設定が必要な場合のみ `continueInForeground()` でアプリを開くパターンを実装する。
