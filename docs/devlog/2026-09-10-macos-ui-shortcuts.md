...
メニュー項目の `Button(intent:)` が**実際に intent を実行するところ**は、クリックを合成できない
環境のため自動では確認していない（`osascript` でのキー送出は使わない方針）。ビルドとメニューの
生成（やること メニューがバーに出て、項目とショートカットが並ぶこと）までは確認済み。
手での確認は #30 のチェックリストに追加した。
