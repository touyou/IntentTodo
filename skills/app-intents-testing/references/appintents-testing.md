...
- **Do not `XCTSkip` when the App Intents stack is unreachable — fail.** A skipped run reports `TEST SUCCEEDED`, so a bundle where nothing executed is indistinguishable from a passing one. Classify instead: a `AppIntentsServicesSecurityErrorDomain` error is a build misconfiguration that waiting cannot fix, so fail immediately rather than burning the timeout once per case; everything else can be polled, but still fails on timeout.
- **Never pass `CODE_SIGNING_ALLOWED=NO` to `test`.** It skips re-signing the UI test runner, which then keeps the `com.apple.XCTRunner` template identity instead of `<app-bundle-id>.xctrunner`. AppIntentsTesting verifies that binding and rejects the run with `AppIntentsServicesSecurityErrorDomain Code=803 "Unable to run internal tests on a Customer build"`, so nothing in the bundle can be verified [measured 2026-09-10, iOS 27 / Xcode 27 RC 27A266a].
