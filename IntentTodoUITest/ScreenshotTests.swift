...
//  to each screen and attaches a full-screen image. `scripts/capture_screenshots.sh` runs it
//  per destination and locale and extracts the attachments.
//
...
final class ScreenshotTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        app = XCUIApplication()
        // The language comes from `xcodebuild -testLanguage`, not from `-AppleLanguages`
        // launch arguments. The other UI tests pin English that way because they match on
        // English accessibility labels; here the whole point is to run the app in each
        // locale, and `-testLanguage` is the switch that also moves `Locale.current` inside
        // the app — which is what picks the fixture's language.
        app.launchArguments = [
            "-uitest-ephemeral-store",
            "-uitest-screenshot-fixture",
            // macOS restores the previous session's windows. If the app was last quit with
            // its window closed it comes back with **no window at all** — the accessibility
            // tree is a menu bar and nothing else, and every element lookup times out.
            // Ignored on the other platforms.
