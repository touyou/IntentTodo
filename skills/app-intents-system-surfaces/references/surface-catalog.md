...
| Spotlight top hits / actions | `IndexedEntity` + intents; `OpenIntent` to open a result | index on mutation; the semantic `indexingKey:` form is iOS/macOS only | [Apple] (`app-intents-entities-and-search`) |
...
Known not to be adoptable as of Xcode 27 beta 6 [measured]: `RelevantEntities.shared.updateEntities(_:for:)` needs an `AppEntityContext`, and the vended contexts are domain-specific (`.audio(.nowPlaying)`, `.audio(.workout(activityType:))`, plus framework-overlay contexts such as HealthKit). There is **no general context**, so a list/todo/document entity has nothing correct to donate under. If contextual suggestion matters, look at `RelevantIntent` / `RelevantIntentManager` (widget-configuration based) instead — a different axis. Recheck when Apple adds domain contexts.
